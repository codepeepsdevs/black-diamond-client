  ...
  return (
    <div className={cn("text-[#A3A7AA] mt-10", isActive ? "block" : "hidden")}>
      ...
      <div className="mt-12 overflow-x-auto whitespace-nowrap">
        ...
        {addonsQuery.isError && <p>Error loading addons list..</p>}
        {addonsQuery.isSuccess && addonsQuery.data?.data.length < 1 && (
          <p>No addon created for this event</p>
        )}
        {addonsQuery.data?.data.map((addon) => {
          ...
          return (
            <div
              key={addon.id}
              className="border-b border-b-[#151515] flex font-medium py-4 [&>div]:px-5"
            >
              <Image
                src={addon.image}
                alt=""
...
function CreateAddonDialog({ ...props }: ComponentProps<typeof Dialog>) {
  ...
  function onError(e: AxiosError<Error>) {
    const errorMessage = getApiErrorMessage(
      e,
      "Something went wrong while creating event addon"
    );
